...
class SaapPortal(GenericContainerContext):
    ...
    def edit_or_report_mode(self, sender, app_data, user_data):
        ...
        #self.average = self.connection.cur_execute(self.tag, query, save=False, database='db_capstone')[1][0]
...
{user_data}'s transaction volume yearly high ${state_ymax:,}
    ...
    def window(self):
        # main window
        with dpg.window(label='SaaP Bank Data-Mart Portal', height=1035, width=1546, tag=self.tag):
            with dpg.group():
                with dpg.child_window():
                    with dpg.tab_bar():
                        # Transactions 
